...
describe('MetaService owner-echo takeover', () => {
  ...
  beforeEach(() => {
    ...
    service = new MetaService(
      ...
      { withDisclosure: jest.fn().mockImplementation(async (_b: string, _c: string, t: string) => t) } as any, // aiDisclosure (pass-through)
      { isIntegrationActive: jest.fn().mockResolvedValue(true) } as any, // integrationSchedule (always active)
    );
